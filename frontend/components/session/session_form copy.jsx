...
class SessionForm extends React.Component {
  ...
  normalizeDate(e) {
    const date = new Date(
      `${this.state.dob.month} ${this.state.dob.day}, ${this.state.dob.year}`
    );
    return () => {
  ...
  toggleEmpty(e) {
    return () => {
      ...
      this.errors =
        this.emptyfield.email === false && this.emptyfield.password === false
          ? true
          : false;
    };
  }
  ...
  render() {
    ...
    const regisButton =
      this.props.formType === "signup" ? (
        ...
        <div className="registration">
          ...
          <Link className="regist-link" to={"/signup"}>
            Register
          </Link>
        </div>
      );
    ...
    const submitButton =
      this.props.formType === "signup" ? (
        ...
        <button onClick={this.toggleEmpty()} className="submit-btn">
          Login
        </button>
      );

    const sessionFormType =
      this.props.formType === "signup" ? "signup-form" : "login-form";

    const formBoxType =
      this.props.formType === "signup" ? "signup-box" : "login-box";
    ...
    const qrCode =
      this.props.formType === "signup" ? null : (
        <div className="demo-user">
          ...
          <h3>
            Scan this with the <strong>Amica mobile app</strong> to log in
            instantly{" "}
          </h3>
        </div>
      );
    ...
    const tos =
      this.props.formType === "signup" ? (
        <p className="tos">
          By registering, you agree to Amica's <span>Terms of Service</span> and{" "}
      ...
      <div className="session-form-container">
        <div className={formBoxType}>
          <form onSubmit={this.handleSubmit} className={sessionFormType}>
            ...
            {usernameInput}
            <label className={`password-label ${toggleRedPass}`}>
              PASSWORD {renderPasswordErr} {this.renderErrors("password")}
              <input
                type="password"
